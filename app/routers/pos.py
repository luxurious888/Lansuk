...
async def checkout_with_discount(body: dict, db: AsyncSession = Depends(get_db)):
    ...
    # ตั้ง session เป็น pending_payment — ยังไม่ปิด
    session.payment_status         = "pending_payment"
    session.pending_payment_method = body.get("payment_method", "cash")
    session.pending_food_disc      = float(food_discount_pct)
    session.pending_total_disc     = float(total_discount_pct)
    session.pending_fixed_disc     = float(fixed_discount)

    return {
        "message":        "เช็คบิลสำเร็จ — รอลูกค้าชำระ",
...
# ── POST /api/pos/order-items/{id}/cancel ─────────────────────────────────────
@router.post("/order-items/{item_id}/cancel")
async def cancel_order_item(item_id: int, body: dict, db: AsyncSession = Depends(get_db)):
    """
    ยกเลิกรายการ (ทั้งหมดหรือบางส่วน) — เก็บ log ไว้ทุกครั้ง
    body: { qty: int, reason: str, cancelled_by: str }
    """
    from datetime import datetime, timezone
    from sqlalchemy import select
    from sqlalchemy.orm import selectinload
    ...
    by     = (body.get("cancelled_by") or "").strip() or "ไม่ระบุ"

    if qty <= 0:
        raise HTTPException(status_code=400, detail="จำนวนที่ยกเลิกต้องมากกว่า 0")
    if not reason:
        raise HTTPException(status_code=400, detail="กรุณาระบุเหตุผลในการยกเลิก")

    result = await db.execute(
        select(OrderItem)
        .where(OrderItem.id == item_id)
        .options(selectinload(OrderItem.order).selectinload(Order.session))
    )
    item = result.scalar_one_or_none()
    if not item:
        raise HTTPException(status_code=404, detail="ไม่พบรายการนี้")

    if item.order and item.order.session:
        if item.order.session.is_paid:
            raise HTTPException(status_code=400, detail="บิลชำระแล้ว ไม่สามารถยกเลิกได้")
        if getattr(item.order.session, "payment_status", "active") == "pending_payment":
            raise HTTPException(status_code=400, detail="บิลรอชำระ — กรุณายกเลิกเช็คบิลก่อน")

    current_cancelled = item.cancelled_qty or 0
    remaining = item.quantity - current_cancelled
    if qty > remaining:
        raise HTTPException(
            status_code=400,
            detail=f"ยกเลิกเกินจำนวนที่เหลือ (เหลือ {remaining})"
        )

    # ต่อเหตุผลใหม่กับเก่า (ถ้ามี) เพื่อเก็บ history
    now = datetime.now(timezone.utc)
    ts = now.strftime("%d/%m %H:%M")
    new_entry = f"[{ts}] -{qty} โดย {by}: {reason}"
    item.cancel_reason = (item.cancel_reason + "\n" + new_entry) if item.cancel_reason else new_entry
    item.cancelled_qty = current_cancelled + qty
    ...
    return {
        ...
        "remaining_qty":  item.quantity - item.cancelled_qty,
    }


# ── POST /api/pos/checkout/confirm ────────────────────────────────────────────
@router.post("/checkout/confirm")
async def checkout_confirm(body: dict, db: AsyncSession = Depends(get_db)):
    """ยืนยันรับเงินจริง — ปิด session"""
    from datetime import datetime, timezone
    from decimal import Decimal
    from sqlalchemy import select
    from sqlalchemy.orm import selectinload
    ...
    if not session:
        raise HTTPException(status_code=404, detail="ไม่พบ session")
    if getattr(session, "payment_status", "active") != "pending_payment":
        raise HTTPException(status_code=400, detail="session นี้ยังไม่ได้เช็คบิล")

    food_pct  = Decimal(str(session.pending_food_disc or 0))
    total_pct = Decimal(str(session.pending_total_disc or 0))
    fixed     = Decimal(str(session.pending_fixed_disc or 0))
    pm        = session.pending_payment_method or "cash"

    for order in session.orders:
        if order.status == OrderStatus.CANCELLED:
            continue
        subtotal = Decimal(str(sum(float(i.line_total) for i in order.items)))
        food_disc  = subtotal * food_pct / 100
        total_disc = (subtotal - food_disc) * total_pct / 100
        all_disc   = food_disc + total_disc + fixed

        order.subtotal       = subtotal
        order.discount_amt   = all_disc
        order.vat_amt        = (subtotal - all_disc) * Decimal("0.07")
        order.total          = subtotal - all_disc + order.vat_amt
        order.payment_method = PaymentMethod(pm)
        order.paid_at        = datetime.now(timezone.utc)
    ...
    return {"message": "ยืนยันรับเงินสำเร็จ", "session_id": session_id}


# ── POST /api/pos/checkout/cancel ─────────────────────────────────────────────
@router.post("/checkout/cancel")
async def checkout_cancel(body: dict, db: AsyncSession = Depends(get_db)):
    """ยกเลิกการเช็คบิล — กลับเป็น active"""
    session_id = body["session_id"]
    session = await db.get(TableSession, session_id)
    if not session:
        raise HTTPException(status_code=404, detail="ไม่พบ session")
    if getattr(session, "payment_status", "active") != "pending_payment":
        raise HTTPException(status_code=400, detail="session นี้ไม่ได้รอชำระ")

    session.payment_status         = "active"
    session.pending_payment_method = None
    session.pending_food_disc      = 0
    session.pending_total_disc     = 0
    session.pending_fixed_disc     = 0

    return {"message": "ยกเลิกเช็คบิลสำเร็จ", "session_id": session_id}
